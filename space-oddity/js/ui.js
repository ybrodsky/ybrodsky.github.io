...
const UI = (() => {
  ...
  function renderTopBar(now) {
    ...
    el("top-location").textContent = SCENE_NAMES[deriveScene(now)];
    ...
    if (mission.type === "courier") {
      ...
      const labels = {
        departing: "Outbound — " + MISSIONS[mission.materialId].sceneName,
    ...
    timerEl.textContent = formatTime(missionTimeRemaining(mission, now)) + " remaining";
  }
  ...
  function renderShipStatus() {
    ...
    el("ship-status-body").innerHTML = `
      ...
      <div class="stat-row unlocks"><span>Unlocked</span><span>${unlockedMaterials().map((m) => MATERIALS[m].name).join(", ")}</span></div>
    `;
  }
  ...
  function renderMissionSelect() {
    const rows = MATERIAL_IDS.map((id) => {
      ...
      const mat = MATERIALS[id];
      const unlocked = GameState.player.ship.drillLevel >= cfg.requiredDrillLevel;
  ...
  function renderCourierOffers() {
    return GameState.courierOffers.map((offer) => {
      ...
      const duration = formatDuration(offer.durationSeconds * reactorMultiplier());
  ...
  function renderActiveCourier(now) {
    ...
    const pct = Math.round(clamp((now - mission.startedAt) / total, 0, 1) * 100);
  ...
  function renderActiveMission(now) {
    ...
    const phase = missionPhase(mission, now);
    const phaseLabels = { departing: "Outbound", mining: "Mining", returning: "Returning", complete: "Docking" };
  ...
  function updateActiveMission(now) {
    ...
    if (mission.type === "courier") {
      const total = mission.returnEndsAt - mission.startedAt;
      const pct = Math.round(clamp((now - mission.startedAt) / total, 0, 1) * 100);
      phaseEl.textContent = missionPhase(mission, now) === "complete" ? "Docking" : "Travelling";
    ...
    el("am-timer").textContent = formatTime(missionTimeRemaining(mission, now));
    el("am-cargo").textContent = cargo + " / " + mission.expectedYield + " units";
    el("am-bar").style.width = Math.round((cargo / mission.expectedYield) * 100) + "%";
  }
  ...
  function renderUpgradePart(part) {
    ...
    if (lvl >= 3) {
      ...
      const matRows = Object.entries(cost.materials).map(([mat, qty]) => {
  ...
  function renderRightPanel(now) {
    ...
    if (GameState.selectedPanel === "mine") {
      html = GameState.currentMission ? renderActiveMission(now) : renderMissionSelect();
    ...
    document.querySelectorAll("#panel-tabs .tab").forEach((b) => {
      b.classList.toggle("tab-active", b.dataset.tab === GameState.selectedPanel);
    });
  }
  ...
  function flashError(reason) {
    showToast(`<div class="toast-title accent-red">ACTION BLOCKED</div><div>${reason}</div>`);
  }
  ...
  function bindEvents() {
    ...
    el("right-widget-body").addEventListener("click", (e) => {
      ...
      if (btn.dataset.startMission) {
        ...
        Scene.playDeparture(() => Scene.setScene("transit", false));
        Scene.setShipMode("flying");
        renderAll();
        ...
        const res = btn.dataset.sell ? sellMaterial(id, qty) : buyMaterial(id, qty);
        if (!res.ok) return flashError(res.reason);
        renderAll();
    ...
    const partMap = { "part-drill": "drill", "part-cargo": "cargo", "part-reactor": "reactor" };
    ...
    el("btn-reset").addEventListener("click", () => {
      ...
      resetGame();
      Scene.setScene("station");
      Scene.setShipMode("docked");
      renderAll();
    });
  }

  return { renderAll, updateActiveMission, renderTopBar, showToast, bindEvents };
})();
